...
public class IndexFragment extends Fragment  {
    ...
    private TextView btnWashCarRecords;
    ...
    public void initView(){
        ...
        btnWashCarRecords = (TextView)view.findViewById(R.id.toolbar_right_tv);
        btnWashCarRecords.setClickable(true);
        ...
        btnWashCar.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(customerInfoBean==null){
                    Toast.makeText(activity,"请先扫码",Toast.LENGTH_SHORT).show();
                    return;
                }
        ...
        switch (requestCode) {
            case Constant.REQ_PERM_CAMERA:
                ...
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    // 被禁止授权
                    Toast.makeText(activity, "请至权限中心打开本应用的相机访问权限", Toast.LENGTH_LONG).show();
                }
                break;
        }
    }
    ...
    public Boolean checkState() {
        if (customerInfoBean.getData().getId() == 0) {
            Toast.makeText(activity,"没获取到顾客id，先扫码获取！",Toast.LENGTH_SHORT).show();
            return false;
        } else {
            return true;
        }
    ...
    private void getCustomerInfoByQRCodeRequest(String qrcode) {
        ...
        WebServiceHelp mServiceHelp = new WebServiceHelp(activity,"iPadService.asmx", "loadData", PubData.class,false,"2");
        mServiceHelp.setOnServiceCallBackString(new WebServiceHelp.OnServiceCallBackString<String>() {
            ...
            public void onServiceCallBackString(boolean haveCallBack, String json) {
                ...
                message.what=111;message.obj = data;
                    ...
                    Toast.makeText(activity,"存在异常 二维码不可识别 code="+customerInfoBean.getCode()+"",Toast.LENGTH_SHORT).show();return;
                }
            }
        });
        mServiceHelp.start(resMap,activity);
    }
    ...
    private void openBillForMemberRequest() {
        ...
        WebServiceHelp mServiceHelp = new WebServiceHelp(activity,"iPadService.asmx", "loadData", PubData.class,true,"2");
        mServiceHelp.setOnServiceCallBackString(new WebServiceHelp.OnServiceCallBackString<String>() {
            @Override
            public void onServiceCallBackString(boolean haveCallBack, String json) {
                Gson gson = new Gson();
                ...
                if( data.getOCODE().equals("00")){
                    changeButtonState();
                    Toast.makeText(activity,"洗车开单成功!",Toast.LENGTH_SHORT).show();
                }else if(data.getOCODE().equals("99")){
                    Toast.makeText(activity,"您不要重复开单！"+data.getOCODE(),Toast.LENGTH_SHORT).show();
                }else{
                    Toast.makeText(activity,"未知错误 code="+data.getOCODE(),Toast.LENGTH_SHORT).show();
                }
            }
        });
        mServiceHelp.start(resMap,activity);
        ...
        mServiceHelp.setOnServiceCallBackString(new WebServiceHelp.OnServiceCallBackString<String>() {
            ...
            public void onServiceCallBackString(boolean haveCallBack, String json) {
                ...
                if (code.equals("00")) {
                    ...
                    Toast.makeText(activity, "重复结算！", Toast.LENGTH_SHORT).show();
                }else{
                    Toast.makeText(activity, "结算失败，未知错误！ code"+code, Toast.LENGTH_SHORT).show();
                    return;
                }
            }
        });
